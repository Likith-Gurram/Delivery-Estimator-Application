...
def detailsview():
    ...
    def verify():
        ...
        if res[0]["status"] == "verified":
            ...
            input_variable = [
                [fromPs.get()],
                [res[0]['matched_address']['postal_code']],
                [res[0]['matched_address']['address_line1']],
                [res[0]['matched_address']['city_locality']],
                [res[0]['matched_address']['state_province']],
                wtE.get()
            ]
    ...
    def show_about():
        messagebox.showinfo("About", "Put your about information here")

    def show_contact_us():
        messagebox.showinfo("Contact Us", "Put your contact information here")
    ...
    def start_progress():
        ...
        for i in range(100):
            progress['value'] = i  # Update the progress bar value
            root.update()  # Refresh the window to show the updated progress bar
            time.sleep(0.001)  # Add a small delay to visualize the progress
        progress['value'] = 0  # Reset the progress bar value after completion
        root.destroy()
